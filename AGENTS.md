...
- JSDoc tag descriptions wrap to the 80-character block-comment width minus indentation (`comments/utils/jsdoc.js`): 73 characters for a top-level, unindented comment, narrower when the comment itself is indented. Run `--fix` instead of adding manual line breaks.
